...
// binarySearchFirst for vector
template<typename T>
int binarySearchFirst(std::vector<T> vec, T target) {
    int index = binarySearch(vec, target); // Find an occurrence of the target
...
// binarySearchFirst for std::array
template<typename T, std::size_t N>
int binarySearchFirst(std::array<T, N> arr, T target) {
    int index = binarySearch(arr, target); // Find an occurrence of the target
    if (index == -1) return -1; // Target not found
